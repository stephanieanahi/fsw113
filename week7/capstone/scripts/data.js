...
function getSystems(parentID,dropDown) {
    fetch('data.json')
    .then(function(response){
        return response.json()
    })
    .then (function(data){
        var filterdData = data.systems.filter(function(sys){
            if (sys.parentID == parentID) {
                return sys
            }
        })
        populateDD(filterdData,dropDown)
    })
...
function populateDD(data,dropDown) {
    ...
    data.forEach(sys => {
        ...
        option.textContent = sys.sysName
        option.value = sys.sysID
...
var secondDropdown = document.querySelector('#subSystem')
 getSystems(value,secondDropdown)
}
)
 // This eventListener responds to a change to the "system" select element by passing the selected value from the "system" element 
    ...
    // appropriate sub-systems from the data.json file.
     var firstDropDown = document.querySelector('#system')
    getSystems(0,firstDropDown)